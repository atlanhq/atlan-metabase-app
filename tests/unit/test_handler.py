...
class TestMetabaseHandlerTestAuth:
    ...
    async def test_auth_failure_raises_when_no_token(self, handler, mock_client):
        """An untyped failure reports the host as unreachable, as preflight does."""
    ...
    async def test_auth_rejected_session_reports_the_auth_failure(
        ...
        mock_client.test_connection = AsyncMock(side_effect=MetabaseSessionAuthError())
    ...
    async def test_auth_no_client_raises(self, handler_no_client):
        ...
        assert result.status == AuthStatus.FAILED
        assert result.message == "Metabase client not initialized"
